...
pub async fn get_startup_items() -> Result<Vec<StartupItem>, String> {
    ...
    if let Ok(run_key) = hkcu.open_subkey_with_flags(RUN_KEY, KEY_READ) {
        let approved_key = hkcu
            .open_subkey_with_flags(STARTUP_APPROVED_RUN, KEY_READ)
            .ok();
    ...
    if let Ok(app_data) = env::var("APPDATA") {
        let startup_path =
            Path::new(&app_data).join(r"Microsoft\Windows\Start Menu\Programs\Startup");
        let hkcu_approved = hkcu
            .open_subkey_with_flags(STARTUP_APPROVED_FOLDER, KEY_READ)
            .ok();
    ...
    if let Ok(wmi_con) = WMIConnection::new() {
        ...
        let query =
            "SELECT Name, DisplayName, PathName, State FROM Win32_Service WHERE StartMode='Auto'";
...
pub async fn set_startup_item_state(id: String, enabled: bool) -> Result<(), String> {
    ...
    let (root, _is_hklm, name_or_file, is_folder) = if id.starts_with("HKCU_RUN_") {
        (
            ...
            false,
            id.trim_start_matches("HKCU_RUN_").to_string(),
            false,
        )
        ...
        (
            ...
            true,
            id.trim_start_matches("HKLM_RUN_").to_string(),
            false,
        )
        ...
        (
            RegKey::predef(HKEY_CURRENT_USER),
            false,
            format!("{}.lnk", id.trim_start_matches("USER_STARTUP_FOLDER_")),
            true,
        )
        ...
        (
            RegKey::predef(HKEY_LOCAL_MACHINE),
            true,
            format!("{}.lnk", id.trim_start_matches("ALL_USERS_STARTUP_FOLDER_")),
            true,
        )
    } else {
    ...
    approved_key
        .set_raw_value(&name_or_file, &reg_value)
