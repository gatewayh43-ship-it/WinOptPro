...
pub async fn kill_process(pid: u32) -> Result<bool, String> {
    ...
    if let Some(process) = sys.process(sys_pid) {
        if process.kill() {
            ...
            return Err(
                "Failed to send kill signal. Permission denied or process elevated.".to_string(),
            );
        }
    }
...
pub async fn set_process_priority(pid: u32, priority: String) -> Result<bool, String> {
    ...
    if let Some(proc_name) = get_process_name_by_pid(pid) {
        ...
        {
            return Err(format!(
                "Cannot modify priority of protected system process: {}",
                proc_name
            ));
        }
    }
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        if !output.status.success() {
            ...
            return Err(format!(
                "Access Denied: You may need Admin privileges. ({})",
                err.trim()
            ));
        }
...
pub async fn open_file_location(pid: u32) -> Result<bool, String> {
    ...
    if let Some(process) = sys.process(sys_pid) {
        if let Some(exe_path) = process.exe() {
            ...
            return Err(
                "Executable path not available (Admin privileges may be needed).".to_string(),
            );
        }
    }
