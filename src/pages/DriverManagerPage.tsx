        ...
        <div className="flex flex-col h-full overflow-hidden p-6 max-w-[1200px] mx-auto space-y-6">
            ...
            <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                ...
                <div className="flex flex-wrap gap-2">
                    ...
                    >
                        {isScanningUpdates ? <Loader2 className="w-4 h-4 animate-spin" /> : <RefreshCw className="w-4 h-4" />}
                        Scan Driver Updates
                    </button>
                    <button
                        onClick={openDriverUpdates}
                        disabled={isOpeningUpdates}
                        className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-primary text-white hover:opacity-90 font-bold text-sm transition-opacity disabled:opacity-50"
                    >
                        {isOpeningUpdates ? <Loader2 className="w-4 h-4 animate-spin" /> : <Download className="w-4 h-4" />}
                        Open Driver Updates
                    </button>
                    <button
                        onClick={() => exportList(savePath)}
                        disabled={isLoading || allDrivers.length === 0}
                        data-testid="export-drivers-json"
                        className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-border bg-card hover:bg-white/5 font-bold text-sm transition-colors disabled:opacity-50"
            ...
            {driverUpdateStatus && (
                <div className="flex items-start gap-3 rounded-xl border border-primary/20 bg-primary/10 p-3 text-sm text-slate-600 dark:text-slate-200">
                    <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-primary" />
                    <span>{driverUpdateStatus}</span>
                </div>
            )}
